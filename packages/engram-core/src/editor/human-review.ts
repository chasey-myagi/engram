...
/**
 * 落一条人审背书（claim_verification, kind='patrol'，带 humanReview 字段）。授权读 actor.isHuman（受信边界）——
 * f1 是「只人能投」的因子，agent（含 role 伪装成 'human:fake' 的 agentActor）一律拒（红线#2：只人能放松；
 * 人审背书亦只人可投）。落库 by_role 写 actor.role（审计身份）。
...
export async function writeHumanReview(
  ...
  if (!opts.actor.isHuman) {
    throw new Error(
      `writeHumanReview: human review must be cast by a human caller (by_role '${opts.actor.role}' is not human)`,
    )
  }
  const id = randomUUID()
