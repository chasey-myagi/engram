 ...
 * 加①这一阶：
 *   - **①是人专属**：caller 必是受信的人（读 `actor.isHuman`，EGR-CR-002）；agent caller **被代码拒**
 *     （不只是文档约定，也不再吃裸字符串前缀——红线#2「只人能放松/裁定」在任何副作用前硬执行受信判据）。
...
export async function humanAdjudicateConflict(
  ...
  // 红线#2 硬执行：①人工裁定仅人可用，授权读受信 actor.isHuman（不吃裸字符串前缀，EGR-CR-002）——
  // agent caller 在任何副作用（边/事件/采信标记）之前即被拒。
