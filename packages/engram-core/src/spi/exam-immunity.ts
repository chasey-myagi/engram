 ...
 * 一把原子落，且事务最开始对候选行加 FOR UPDATE 锁 + 锁内复核 status（EGR-CR-020）——串行化并发决定，
 * 杜绝并发 pass+fail 造出「golden 行 ∧ 候选 rejected」的矛盾终态。故若决定事务因未抢到占有权早退（或失败），
 * 已提交的毒株是 draft 孤儿——永不被召回、永不进 golden，惰性无害。
 */
...
export async function promoteCandidate(
  ...
  return db.transaction(async (tx) => {
    // 防 TOCTOU + lost-update（EGR-CR-020）：事务外的 :100 预读只作早退优化、不是权威。这里对候选行加
    // FOR UPDATE 锁并在锁内复核 status——串行化并发决定，杜绝「pass 写 golden + fail blind UPDATE 改回
    // rejected ⇒ golden 行 ∧ rejected 终态」的矛盾。对齐同库 transition/commit-claim 的 .for('update') 范式。
    ...
    if (!locked || locked.status !== 'queued') {
      // 没抢到 queued 占有权：另一并发调用已把它落终态 ⇒ already-decided，不写 golden / 不 blind UPDATE /
      // 不写决定审计。本次已造的毒株 draft 成孤儿——与文件头论证的「draft 孤儿惰性无害」语义一致，无需回收。
      throw new Error(
