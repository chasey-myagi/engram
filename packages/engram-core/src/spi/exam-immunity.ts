 ...
 * 一把原子落。决定事务最开始对候选行加 FOR UPDATE 锁并复核 status（对齐 transition / commit-claim 范式）：
 * 并发 pass+fail 时，第二个调用读到非 queued ⇒ already-decided 早退、不写任何东西，杜绝 (golden 行 ∧ rejected) 矛盾终态。
 * 故若决定事务因 already-decided 早退/并发撞 golden 的 candidate_id UNIQUE 而不晋升，已提交的毒株是 draft 孤儿
 * ——永不被召回、永不进 golden，惰性无害。
 */
...
export async function promoteCandidate(
  ...
  return db.transaction(async (tx) => {
    ...
    // commit-claim.ts 的「锁行 + 事务内复核」范式）。事务外那道 status 预读只是早退优化、非权威：并发的另一笔
    // 决定可能已在它之后把候选落终态。第二个调用在此 .for('update') 后读到非 queued ⇒ already-decided 早退，
