...
export type { ItemAdmission } from './a1-admission.js'
import {
  scoreAdmittedGeneration,
  type ClassScore,
  type InjectionOutcome,
} from './redteam-injector.js'
...
export async function runRedBlueRound(
  ...
  // EGR-CR-019：编排层已在②步自跑 A1，故直接调**内部**scoreAdmittedGeneration（纯打分、不再自带 A1），
  // 避免与公开 runRedTeamGeneration 的内建 A1 各跑一遍（同一条 admitViaA1 路径、零重复）。
  const classScores =
    scoredItems.length === 0
      ? []
      : await scoreAdmittedGeneration(deps, scoredItems, opts.resetWorkTables)
